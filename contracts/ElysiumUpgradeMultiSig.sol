...
contract ElysiumUpgradeMultiSig is ReentrancyGuard, AccessControl {
    ...
    constructor(address[] memory _initialSigners, address _admin, address _citizenshipNft) {
        require(_initialSigners.length <= MAX_SIGNERS, "UpgradeMultiSig: too many signers");
        require(_initialSigners.length >= REQUIRED_SIGNATURES, "UpgradeMultiSig: too few signers");
    ...
    function _checkEligibility(address signer) internal view returns (bool) {
        ...
        // H1 only (phase 1)
        if (citizen.phase != 1) return false;
        return true;
    }

    function _addSigner(address signer) internal {
        require(!isSigner[signer], "UpgradeMultiSig: already a signer");
        require(_checkEligibility(signer), "UpgradeMultiSig: ineligible signer");
    ...
    function _removeSigner(address signer) internal {
        require(isSigner[signer], "UpgradeMultiSig: not a signer");
    ...
    function submitTransaction(address to, uint256 value, bytes calldata data) external onlyRole(SIGNER_ROLE) {
        require(to != address(0) || value > 0 || data.length > 0, "UpgradeMultiSig: invalid transaction");
    ...
    function confirmTransaction(uint256 txId) external onlyRole(SIGNER_ROLE) {
        ...
        require(txn.value > 0 || txn.to != address(0) || txn.data.length > 0, "UpgradeMultiSig: invalid transaction");
        require(!txn.executed, "UpgradeMultiSig: already executed");
        require(!isConfirmed[txId][msg.sender], "UpgradeMultiSig: already confirmed");
        isConfirmed[txId][msg.sender] = true;
        txn.approvals += 1;
        emit TransactionConfirmed(txId, msg.sender);
    }
    ...
    function executeTransaction(uint256 txId) external nonReentrant onlyRole(SIGNER_ROLE) {
        Transaction storage txn = transactions[txId];
        require(!txn.executed, "UpgradeMultiSig: already executed");
        require(txn.approvals >= REQUIRED_SIGNATURES, "UpgradeMultiSig: not enough confirmations");
        (bool success, ) = txn.to.call{value: txn.value}(txn.data);
        require(success, "UpgradeMultiSig: execution failed");
        txn.executed = true;
